...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Buscar usuario
    $stmt = $pdo->prepare("SELECT * FROM familiares WHERE correo_electronico = :email LIMIT 1");
    ...
    $stmt = $pdo->prepare("DELETE FROM sesiones_familiares WHERE id_usuario = :id_usuario");
    ...
    $stmt = $pdo->prepare("INSERT INTO sesiones_familiares (id_usuario, token) VALUES (:id_usuario, :token)");
    $stmt->execute([
        ':id_usuario' => $user['id'],
        ':token' => $token
    ]);
    ...
    $stmt = $pdo->prepare("UPDATE familiares SET activo = 1 WHERE id = :id");
    ...
    header('Location: ../HTML/dashboard_familiar.html');
    exit;
}
?>
