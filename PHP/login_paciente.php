...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Buscar usuario
    $stmt = $pdo->prepare("SELECT * FROM pacientes WHERE correo_electronico = :email LIMIT 1");
    ...
    $stmt = $pdo->prepare("DELETE FROM sesiones_pacientes WHERE id_usuario = :id_usuario");
    ...
    $stmt = $pdo->prepare("INSERT INTO sesiones_pacientes (id_usuario, token) VALUES (:id_usuario, :token)");
    $stmt->execute([
        ':id_usuario' => $user['id'],
        ':token' => $token
    ]);
    ...
    $stmt = $pdo->prepare("UPDATE pacientes SET activo = 1 WHERE id = :id");
